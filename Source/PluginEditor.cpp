...
void glowStroke(juce::Graphics& g,const juce::Path& p,juce::Colour c,float width,bool glow){
    if(glow){stroke(g,p,c.withAlpha(.09f),width*4.5f);stroke(g,p,c.withAlpha(.15f),width*2.3f);}
    stroke(g,p,c,width);
}
...
void ModernDial::paint(juce::Graphics& g){
    ...
    if(look.isNeon())for(int i=compact?3:5;i>0;--i){g.setColour(juce::Colour(accent).withAlpha(.035f));g.fillEllipse(face.expanded(float(i)*2*s));}
    if(look.isNeon())g.setGradientFill(juce::ColourGradient(look.pick(0xff233349,0,0),c.x-r,c.y-r,look.pick(0xff060b12,0,0),c.x+r,c.y+r,false));else g.setColour(look.pick(0,0xff252525,0xfff8f8f8));g.fillEllipse(face);g.setColour(look.pick(0xff344963,0xff555555,0xffbabcc0));g.drawEllipse(face,s);
...
DuckPocketAudioProcessorEditor::DuckPocketAudioProcessorEditor(DuckPocketAudioProcessor& p):AudioProcessorEditor(&p),audioProcessor(p){
    ...
    auto saved=preferences->getValue("duckPocket.ui.theme",preferences->getValue("phasePocket.ui.theme","solidDark"));setTheme(saved=="neon"?PocketTheme::Neon:(saved=="solidWhite"?PocketTheme::SolidWhite:PocketTheme::SolidDark),false);
...
void DuckPocketAudioProcessorEditor::setFrozen(bool frozen){
    ...
    if(frozen){
        ...
        for(int i=0;i<filled;++i)frozenGain.push_back(history[size_t((cursor-filled+i+8192)%8192)]);
        frozenScope=frozenGain;
    }else{
        const double resume=filled?history[size_t((cursor+8191)%8192)].time:0.;
...
void DuckPocketAudioProcessorEditor::graph(juce::Graphics& g,juce::Rectangle<float> box,bool gain){
    ...
    for(int kind=1;kind>=0;--kind){
        ...
        juce::Path body;body.startNewSubPath(top.front());for(size_t i=1;i<top.size();++i)body.lineTo(top[i]);for(size_t i=bottom.size();i-->0;)body.lineTo(bottom[i]);body.closeSubPath();
        const auto colour=kind?look.pick(0xff3794d4,0xff8e8e8e,0xff59616b):label;
        if(glow){g.setColour(colour.withAlpha(.09f));g.strokePath(body,juce::PathStrokeType(6.f,juce::PathStrokeType::curved,juce::PathStrokeType::rounded));g.setColour(colour.withAlpha(.15f));g.strokePath(body,juce::PathStrokeType(3.f,juce::PathStrokeType::curved,juce::PathStrokeType::rounded));}
        g.setColour(colour.withAlpha(kind?.3f:.38f));g.fillPath(body);
